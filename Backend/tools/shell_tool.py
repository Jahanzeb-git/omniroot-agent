...
class ShellToolManager:
    ...
    def get_current_workflow_id(self) -> Optional[str]:
        """Fetch current workflow ID from database with retry logic"""
        for attempt in range(self.max_db_retries + 1):
            try:
                ...
                cursor.execute("SELECT workflow_id FROM workflows WHERE session_id = ? ORDER BY start_time DESC LIMIT 1", (session_id,))
                ...
                if attempt < self.max_db_retries:
                    ...
                    raise Exception(f"Database connection failed after {self.max_db_retries} retries: {str(e)}")
    ...
    def execute_command(self, command: str, timeout: int = 60, use_sudo: bool = False, source: str = "agent") -> Dict[str, Any]:
        """Execute shell command with complete isolation between commands and improved server handling"""
        try:
            # Trim the command
            command = command.strip()
            
            # Emit command event to terminal
            terminal_events.emit_command(command, source)
            ...
            if not workflow_id:
                result = {
                    ...
                    "status": "Failed",
                    "output": "Error: Could not retrieve current workflow ID from database",
                    "warnings": ""
                }
                ...
                for command in commands:
                    ...
                    if command.startswith('cd '):
                        ...
                        if os.path.isdir(full_dir_path):
                            current_dir = os.path.normpath(full_dir_path)
                            self.thread_local.current_dir = current_dir
                            output_lines.append(f"Changed directory to: {current_dir}")
                        else:
                            output_lines.append(f"cd: {full_dir_path}: No such file or directory")
                            exit_code = 1
                            break
                    else:
...
""")
            ...
            # Create a special script for cd commands
                ...
                full_dir_path = os.path.join(current_dir, dir_path)
                if os.path.isdir(full_dir_path):
                    normalized_path = os.path.normpath(full_dir_path)
                    self.thread_local.current_dir = normalized_path
                    with open(output_file, 'w') as f:
                        f.write(f"Changed directory to: {normalized_path}")
                    with open(exit_code_file, 'w') as f:
                        f.write("0")
                    with open(pwd_file, 'w') as f:
                        f.write(normalized_path)
                    output = f"Changed directory to: {normalized_path}"
                    terminal_events.emit_output(output)
                    return {
                    ...
                    with open(output_file, 'w') as f:
                        f.write(f"cd: {full_dir_path}: No such file or directory")
                    with open(exit_code_file, 'w') as f:
                        f.write("1")
                    current_dir = getattr(self.thread_local, 'current_dir', self.workspace_dir)
                    with open(pwd_file, 'w') as f:
                        f.write(current_dir)
                    output = f"cd: {full_dir_path}: No such file or directory"
                    terminal_events.emit_output(output)
                    return {
                        "working_directory": current_dir,
                        "status": "Failed",
...
if [ -n "$SHELL_TOOL_CURRENT_DIR" ] && [ -d "$SHELL_TOOL_CURRENT_DIR" ]; then
    cd "$SHELL_TOOL_CURRENT_DIR"
fi
...
""")
            ...
            env = os.environ.copy()
            if hasattr(self.thread_local, 'current_dir'):
                env["SHELL_TOOL_CURRENT_DIR"] = self.thread_local.current_dir
...
# Global instance
shell_manager = ShellToolManager()

def shell_tool_function(input_str: str) -> str:
    """Main shell tool function for Langchain"""
    try:
        input_data = json.loads(input_str)
        command = input_data.get("command", "")
        timeout = input_data.get("timeout", 60)
        use_sudo = input_data.get("use_sudo", False)
        source = input_data.get("source", "agent")  # Can be "agent" or "user"
        
        if not command:
            return "Working Directory: Unknown\nStatus: Failed\nOutput: ---------------------\nError: No command provided\nWarnings or System Message: "
        
        result = shell_manager.execute_command(command, timeout, use_sudo, source)
        
        formatted_output = f"""Working Directory: {result['working_directory']}
Status: {result['status']}
Output: ---------------------
{result['output']}
Warnings or System Message: {result['warnings']}"""
        ...
        terminal_events.emit_output(error_msg)
        return f"Working Directory: Unknown\nStatus: Failed\nOutput: ---------------------\n{error_msg}\nWarnings or System Message: "